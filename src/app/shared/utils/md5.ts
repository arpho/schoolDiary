export function md5(str: string): string {
  ...
  // Add two 32-bit numbers, wrapping around
  ...
  // Convert string to array of blocks
  ...
  // UTF-8 encoding is necessary for Gravatar but keeping it simple for now if inputs are standard emails
  // For robustness, here's a simple UTF8 encoder
  const toUTF8 = (str: string) => {
      let utf8 = "";
      for (let i = 0; i < str.length; i++) {
          const charcode = str.charCodeAt(i);
          if (charcode < 128) utf8 += String.fromCharCode(charcode);
